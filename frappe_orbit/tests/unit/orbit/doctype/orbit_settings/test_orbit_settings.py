...
class TestOrbitSettings(UnitTestCase):
    ...
    def test_custom_authorize_adds_redirect_uri(self):
        from frappe_orbit.integrations.oauth2 import authorize
    ...
    def test_after_install_creates_client(self):
        from frappe_orbit.install import after_install
    ...
    def test_before_uninstall_deletes_client_and_tokens(self):
        from frappe_orbit.uninstall import before_uninstall
        ...
        client_exists = frappe.db.exists("OAuth Client", {"app_name": "Orbit"})
    ...
    def test_get_permission_query_conditions(self):
        from frappe_orbit.integrations.oauth2 import get_permission_query_conditions
        
        # Administrator should see everything
        self.assertEqual(get_permission_query_conditions("Administrator"), "")
        
        # Other users should not see Orbit
        self.assertEqual(get_permission_query_conditions("test@example.com"), "`tabOAuth Client`.app_name != 'Orbit'")

    def test_has_permission(self):
        from frappe_orbit.integrations.oauth2 import has_permission
