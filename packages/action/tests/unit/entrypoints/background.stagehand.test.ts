...
describe('Stagehand Orchestration Flow', () => {
  ...
  it('should handle Cache HIT successfully', async () => {
    ...
    // Step 1: Check Cache
    const cacheRes = await fetch(`${siteUrl}/api/method/frappe_action.action_node_cache.get`, {
      method: 'POST',
  ...
  it('should handle Cache MISS and trigger LLM inference', async () => {
    ...
    // Step 3: Infer Action
    const inferRes = await fetch(`${siteUrl}/api/method/frappe_action.action_node_cache.infer_action`);
    ...
    // Step 5: Save Cache
    const saveRes = await fetch(`${siteUrl}/api/method/frappe_action.action_node_cache.save`);
