...
test.describe('End-to-End Workflow Execution', () => {
  test('Successful workflow completion', async ({ page, context }) => {
    ...
    await page.route('**/api/method/frappe_action.todo.submit', async (route) => {
      await route.fulfill({
        status: 200,
    ...
    await background.evaluate(async () => {
      ...
      for (const tab of tabs) { if (tab.id) {
        chrome.tabs.sendMessage(tab.id, { type: 'TOGGLE_ACTION_UI' }); }
      }
    });
    ...
    const panel = shadowHost.locator('.action-panel');
