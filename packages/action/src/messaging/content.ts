...
export function setupContentMessageListeners(uiRef: { current: any }, isMounted: { value: boolean }) {
  browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.type === "REQUIRE_HITL") {
      ...
      window.dispatchEvent(new CustomEvent("ACTION_REQUIRE_HITL", { detail: message.payload }));
    } else if (message.type === "RUN_ALL_COMPLETED") {
      window.dispatchEvent(new CustomEvent("ACTION_RUN_ALL_COMPLETED"));
  ...
  window.addEventListener("ACTION_START_HEADLESS_ACTION", (event: Event) => {
  ...
  window.addEventListener("message", (event) => {
    ...
    } else if (event.data && event.data.type === "ACTION_CHECK_AUTH_STATUS") {
      browser.runtime.sendMessage({
        type: "CHECK_AUTH_STATUS",
        payload: event.data.payload
      }, (response) => {
        window.postMessage({
          type: "ACTION_AUTH_STATUS_RESULT",
          payload: response
        }, "*");
      });
    }
