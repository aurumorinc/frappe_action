...
export const ACTION_SITES_KEY = "local:action_sites";

export async function getSites(): Promise<SiteAuth[]> {
  const sites = await storage.getItem<SiteAuth[]>(ACTION_SITES_KEY);
  return sites || [];
}
...
export async function saveSite(site: SiteAuth): Promise<void> {
  ...
  await storage.setItem(ACTION_SITES_KEY, sites);
}
