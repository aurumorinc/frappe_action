...
export function transmitTelemetry(level: number, logEvent: any) {
  ...
  const msg = `[Action] ${logEvent.messages[0]}`;
  ...
  if (typeof window !== 'undefined' && (window as any).__ACTION_LOGS__) {
    (window as any).__ACTION_LOGS__.push(logEvent);
  }
