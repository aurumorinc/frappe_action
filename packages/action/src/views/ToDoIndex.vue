<template>
  <div
    v-show="show"
    class="fixed z-[2147483647] right-0 w-80 h-[calc(100%_-_80px)] text-ink-gray-9 m-5 mt-[62px] p-3 flex gap-2 flex-col justify-between rounded-lg bg-surface-modal shadow-2xl"
  ...
  >
    ...
    <div class="h-full overflow-hidden flex flex-col">
      <template v-if="!isOnboardingStepsCompleted && !showSettings">
        <div class="flex flex-col justify-center items-center gap-1 mt-4 mb-7">
          ...
          <div v-if="hasAuthenticatedSites" class="text-p-base font-normal">
        ...
        <div class="flex flex-col gap-2.5 overflow-hidden">
          ...
          <div v-if="!hasAuthenticatedSites" class="text-center text-ink-gray-5 py-4">
            Please connect a site in Settings.
          </div>
          <div v-else-if="isLoading" class="text-center text-ink-gray-5 py-4">
            Loading...
          </div>
          <div v-else-if="visibleActions.length === 0" class="text-center text-ink-gray-5 py-4">
            No pending tasks
          </div>
          <div v-else class="flex flex-col gap-1.5 overflow-y-auto">
            <div
              ...
              :class="{ 'opacity-50': action.completed || isDependent(action) }"
              @click.stop="
...
import ActionLogo from '../icons/ActionLogo.vue'
...
const title = 'Frappe Action'
const logo = markRaw(ActionLogo)
...
onMounted(() => {
  ...
  window.addEventListener("ACTION_REQUIRE_HITL", () => {
    show.value = true; // Ensure UI is visible
    minimize.value = false; // Ensure UI is not minimized
  });

  window.addEventListener("ACTION_RUN_ALL_COMPLETED", () => {
